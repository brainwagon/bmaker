...
The layout engine parses the template, resolves each `Field` reference against the data set, and renders the card. Fields absent from the data set or whose values are empty strings are hidden; the remaining fields in that column reflow vertically.
...
1. Parses the template into a segment/column/field tree
2. Applies the `Theme` CSS class and `FontPair` to the card container
3. Binds data values to field display elements
4. Positions `Logo` and `QRCode` as floating overlays per the active `Theme`
5. Runs the auto-scale pass to fit text within bounds and avoid overlays

---

## 2. Canonical Field Names

Only these field names are valid in a `Field` entry in the layout (§3.4).
...
| `Website` | URL; also used as the QR code source when `QRCode` is `"true"`              | No       |

`Logo` and `QRCode` are not layout fields — they are positioned by the active `Theme` as floating overlays. They are supplied in the data set and controlled via template metadata (§3.1), not placed in segments or columns.
...
| `QRCode`         | `"true"` \| `"false"`                                | No       | `"false"`   | Render a QR code generated from the `Website` field value          |
...
| `Padding`        | dimension value or CSS shorthand     | No       | `"0"`                 | Inner padding applied uniformly or per-side     |
...
| `Items`     | ordered list of `Field` entries (§3.4)      | Yes      | Fields displayed in this column, rendered top to bottom        |
...
Each entry in `Items` names a canonical field (§2) and specifies its typography. Fields appear in the order listed.
...
| Key         | Type / Values                                                   | Required | Default      | Description                                                                   |
|-------------|------------------------------------------------------------------|----------|--------------|-------------------------------------------------------------------------------|
| `Field`     | canonical field name (§2)                                        | Yes      | —            | The data field to render; omitted from output if the data value is empty      |
| `Size`      | `pt` or `px` value (e.g. `"12pt"`, `"16px"`)                    | Yes      | —            | Initial font size; the layout engine may scale this down to prevent overflow  |
| `Style`     | space-separated: `bold` `italic` `normal`                        | No       | `"normal"`   | Font style; `"normal"` must not be combined with `"bold"` or `"italic"`      |
| `Alignment` | `"left"` \| `"center"` \| `"right"`                             | No       | column value | Overrides the column `Alignment` for this field only                          |

The layout engine scales `Size` down in 0.5pt steps to prevent overflow and to avoid collisions with `Logo` and `QRCode` overlays. Font size is never reduced below `8pt`.
